(function (root) {
  ...
  function create(id, node, context) {
    ...
    var sketch = new root.p5(function (p) {
      ...
      p.windowResized = function () {
        var box = node.getBoundingClientRect();
        // Canvas coordinates change with the CSS box. A stale anchor would point
        // at a different frozen row after a breakpoint or orientation change.
        local.mx = -1; local.my = -1; local.clickX = -1; local.clickY = -1;
        local.pressed = false; local.hasPointer = false;
    ...
    return {
      ...
      clearPointerSelection: function () { local.clickX = -1; local.clickY = -1; if (active) { sketch.redraw(); } },
  ...
  function caveat(p, text) {
    p.noStroke(); p.fill("rgba(6,20,31,.82)");
    if (p.width < 680) {
      p.rect(12, p.height - 46, p.width - 24, 34, 4);
      wrap(p, text, 22, p.height - 41, p.width - 44, 26, { color: "#E9E1CC", mono: true, size: 7.5 });
    } else {
      p.rect(12, p.height - 34, p.width - 24, 22, 4);
  ...
  // Return one immutable claim row from the embedded facts digest. Painters use
  // this for evidence labels instead of carrying a second copy of a headline.
  function fact(state, claimId) {
    var rows = state && state.facts && state.facts.claims;
    if (!rows) { return null; }
    for (var i = 0; i < rows.length; i += 1) {
      if (rows[i].claim_id === claimId) { return rows[i]; }
    }
    return null;
  }
  ...
  root.EVO_DRAW = { clamp: clamp, lerp: lerp, smooth: smooth, fade: fade, lineLabel: lineLabel, label: label, wrap: wrap, panel: panel, dot: dot, caveat: caveat, meter: meter, fact: fact, ambient: ambient };
}(window));
