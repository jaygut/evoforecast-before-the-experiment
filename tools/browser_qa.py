...
SCENES = ["hero", "stakes", "blindspot", "windtunnel", "phase", "frontier", "challenge", "genome", "abstain", "ladder", "ask"]
...
def check_page(page, label: str, screenshot: str) -> dict:
    ...
    return {"label": label, "console_errors": console_errors, "remote_requests": remote, "scenes": scene_results, "phase_readout": phase_text, "frontier_readout": frontier_text, "adverse_readout": adverse_text, "horizontal_overflow_px": overflow, "fonts": fonts, "semantics": semantics}
...
def main() -> None:
    ...
    for result in results[:2]:
        ...
        if not all(result["fonts"].values()): failures.append(result["label"] + ": font load")
        if result["semantics"] != {"main": 1, "h1": 1, "sections": 11, "skip": True, "boundary": True, "live": 5, "sourcePills": 11, "caveats": 11}:
            failures.append(result["label"] + ": semantic inventory mismatch " + repr(result["semantics"]))
    if results[-1]["scene_titles"] != 11 or not results[-1]["noscript_visible"] or results[-1]["source_pills"] != 11:
