...
def main() -> int:
    ...
    parser.add_argument(
        "--trajectory-case",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "data" / "trajectory_case.json",
        help="frozen trajectory case generated by export_trajectory_case.py",
    )
    parser.add_argument(
        "--wind-tunnel-brief",
        type=Path,
    ...
    source_trajectory_path = args.trajectory_case.resolve()
    trajectory_path = data_dir / "trajectory_case.json"
    trajectory = json.loads(source_trajectory_path.read_text())
    if trajectory["summary"]["replicate_count"] != 32 or trajectory["summary"]["present_day90"] != 32:
        raise SystemExit("trajectory_case.json fails the registered 32-of-32 persistence invariant")
    if source_trajectory_path != trajectory_path.resolve():
        trajectory_path.write_bytes(source_trajectory_path.read_bytes())
    ...
    claims = [
        ...
        claim("EF-006", f"Zero of {len(v5_frontier)} blind-v5 policies were classified robustly Pareto optimal.", "indicative", "BLIND_V5_FRONTIER", "policy cells", str(len(v5_frontier)), "no robustly Pareto-optimal synthetic policy", "no experiment can work", "Some policies are conditional or fragile. Zero satisfy the robust classification."),
        ...
        claim("EF-016", "V3 through V5 are software and challenge-design maturation snapshots. Biological generations are a separate concept.", "synthetic", "PROMPT", "chronology", "software snapshots", "software/challenge chronology", "biological evolutionary sequence", "Cohorts differ in registered synthetic design and scoring treatment."),
        ...
        claim("EF-018", "In registered Study E cell adaptive_E07, all 32 synthetic vessels retained Daphnia through day 90 while standing selection changed the shares of eight immutable founders.", "synthetic", "RC01_TRAJECTORY_CASE", "replicate vessels", "32", "registered synthetic persistence-cell trajectories", "empirical persistence or transfer", "Its scope is one registered synthetic persistence diagnostic. Empirical persistence and transfer await testing."),
    ]
    ...
    digest = {
        ...
        "snapshot_date": SNAPSHOT_DATE,
        "claim_boundary": "This digest contains synthetic software and experimental-design evidence. Empirical Daphnia forecast skill, GT01/G1 qualification, and external endorsement remain outside its scope.",
        "claims": claims,
    ...
    manifest = {
        ...
        "sources": [
            ...
            "locator": trajectory["provenance"]["source_collection"],
            "basename": "32 adaptive_E07 latent_state.csv members",
            "sha256": trajectory["provenance"]["source_collection_sha256"],
            "bytes": trajectory["provenance"]["source_collection_bytes"],
            "mutable": False,
        }],
    }
    ...
    write_json(audit_dir / "lineage_dag.json", {
        ...
        "nodes": [{"id": s["source_id"], "kind": "immutable_source", "sha256": s["sha256"]} for s in manifest["sources"]] + [
            ...
            {"id": "trajectory_case", "kind": "derived_public_data", "sha256": sha256(trajectory_path)},
        ...
        "edges": [
            ...
            {"from": "RC01_TRAJECTORY_CASE", "to": "trajectory_case", "operation": "exact-row projection"},
            {"from": "trajectory_case", "to": "site", "operation": "lookup-only rendering"},
        ],
    })
